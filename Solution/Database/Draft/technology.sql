...
CREATE OR REPLACE FUNCTION mask_policy(schema_name IN VARCHAR2, table_name IN VARCHAR2) RETURN VARCHAR2 IS
    v_count NUMBER;
BEGIN
    SELECT COUNT(*) INTO v_count
    FROM DBA_ROLE_PRIVS
    WHERE GRANTEE = USER AND GRANTED_ROLE = 'ADMIN_ROLE';

    IF v_count > 0 THEN
        RETURN '1=1'; -- Полный доступ для ролей с ADMIN_ROLE
    ELSIF USER = 'ADMIN_USER' THEN
        RETURN '1=1'; -- Полный доступ для ADMIN_USER
    ELSE
        RETURN '1=0'; -- Ограничение для остальных пользователей
    END IF;
END mask_policy;
...
SELECT * 
FROM USER_ERRORS 
WHERE NAME = 'MASK_POLICY' AND TYPE = 'FUNCTION';
...
BEGIN
    DBMS_RLS.ADD_POLICY(
        ...
        policy_name => 'MASK_POLICY_USER_PROFILE',
        function_schema => 'SYS',
        policy_function => 'mask_policy',
    ...
    DBMS_RLS.ADD_POLICY(
        ...
        policy_name => 'MASK_POLICY_APP_USER',
        function_schema => 'SYS',
        policy_function => 'mask_policy',
        statement_types => 'SELECT'
    );
...
FROM USER_OBJECTS
WHERE OBJECT_TYPE IN ('FUNCTION', 'TRIGGER', 'PACKAGE')
AND STATUS = 'INVALID';

SELECT TRIGGER_NAME, STATUS
FROM USER_TRIGGERS
WHERE TABLE_NAME IN ('USER_PROFILE', 'APP_USER', 'CARD');